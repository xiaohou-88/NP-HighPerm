...
def atom_features(atom):
    ...
    return [
        ...
        degree,                        # Atom degree
        int(is_aromatic),              # Aromatic atom flag
        int(is_hydrogen),              # Hydrogen atom flag
        int(is_in_ring),               # Ring membership flag
    ]


def bond_features(bond):
    bond_type = int(bond.GetBondTypeAsDouble())  # Single, double, or triple bond
...
def smi_tokenizer(smi):
    ...
    if "".join(tokens) != smi:
        raise ValueError(
            f"SMILES tokenization failed:\n"
            f"Original: {smi}\n"
            f"Rebuilt:  {''.join(tokens)}"
        )
...
class MolDataset(Dataset):
    def __init__(self, csv_file, max_len=128, vocab=None, word2id=None, unk_token="UNK"):
        ...
        if vocab is None or word2id is None:
            self.vocab, self.word2id = build_vocab(self.smiles, unk_token=unk_token)
        else:
            self.vocab = list(vocab)
            self.word2id = dict(word2id)
            if unk_token is not None and unk_token not in self.word2id:
                self.word2id[unk_token] = len(self.vocab)
                self.vocab.append(unk_token)
        self.unk_id = self.word2id.get(unk_token) if unk_token is not None else None
    ...
    def generate_combined_fp(self, smiles):
        """Generate combined Morgan and MACCS fingerprint features."""
        mol = Chem.MolFromSmiles(smiles)
        FP_DIM = 2048 + 167
